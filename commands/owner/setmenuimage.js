    ...
    async execute(sock, msg, args, context) {
        ...
        try {
            ...
            if (!ctx?.quotedMessage) {
                return sock.sendMessage(from, { text: '📷 Please reply to an *image* or *sticker* to set it as the menu image.' });
            }
            ...
            if (!imageMsg) {
                return sock.sendMessage(from, { text: '❌ The replied message must be an *image* or *sticker*.' });
            }
            ...
            if (!mediaBuffer) {
                return sock.sendMessage(from, { text: '❌ Failed to download the image. Please try again.' });
            }
            ...
            const imagePath = path.join(__dirname, '../../src/utils/bot_image.jpg');
            ...
            await sock.sendMessage(from, { text: '✅ Menu image has been updated successfully!' });
            
        } catch (error) {
            console.error('SetMenuImage command error:', error);
            await sock.sendMessage(from, { text: `❌ Failed to set menu image: ${error.message}` });
        }
    }
};
