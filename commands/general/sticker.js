/**
 * Sticker Command
 * Convert image or video to sticker with EXIF metadata
 */
    ...
    async execute(sock, msg, args, context) {
        ...
        if (!mediaMessage) {
            return await sock.sendMessage(from, { 
                text: '📎 Reply to an *image* / *video* with .sticker or send media with .sticker as caption.' 
            });
        }
        
        const tmpDir = os.tmpdir ? os.tmpdir() : '/tmp';
        const timestamp = Date.now();
        const tempInput = path.join(tmpDir, `in_${timestamp}`);
        const tempOutput = path.join(tmpDir, `out_${timestamp}.webp`);
        ...
        try {
            ...
            if (mediaBuffer.length > MAX_FILE_SIZE) {
                await sock.sendMessage(from, { 
                    text: `❌ File too large: ${(mediaBuffer.length / 1024 / 1024).toFixed(2)}MB (max: ${MAX_FILE_SIZE / 1024 / 1024}MB)` 
                });
                return;
            }
            ...
            if (isAnimated && webpBuffer.length > 1000 * 1024) {
                const tempOutput2 = path.join(tmpDir, `out_fallback_${Date.now()}.webp`);
            ...
            const json = {
                ...
                'sticker-pack-name': config.packname || 'Made by Giluce',
                emojis: ['🤖'],
            };
            ...
            await sock.sendMessage(from, { 
                text: '❌ Failed to create sticker. Make sure the media is valid and FFmpeg is installed.' 
            });
        } finally {
