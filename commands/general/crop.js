...
module.exports = {
    ...
    async execute(sock, msg, args, context) {
        ...
        const tempInput = path.join(tmpDir, `temp_${timestamp}`);
        const tempOutput = path.join(tmpDir, `crop_${timestamp}.webp`);
        ...
        try {
            ...
            if (!mediaInfo) {
                return await sock.sendMessage(from, { 
                    text: '✂️ Reply to a *sticker*, *image*, or *video* that you want to crop.' 
                });
            }
            ...
            if (!mediaMessage) {
                return await sock.sendMessage(from, { 
                    text: '✂️ Please reply to an image/video/sticker with .crop, or send an image/video/sticker with .crop as the caption.' 
                });
            }
            ...
            await sock.sendMessage(from, { 
                text: '❌ Failed to crop sticker! Try with an image or video.' 
            });
        } finally {
