    ...
    async execute(sock, msg, args, context) {
        ...
        try {
            ...
            if (mode === 'private' || mode === 'priv') {
                if (config.selfMode) {
                    return sock.sendMessage(from, { text: '🔒 Bot is already in *PRIVATE* mode.\nOnly owner can use commands.' });
                }
                
                // Update config
                updateConfig('selfMode', true);
                config.selfMode = true;
                return sock.sendMessage(from, { text: '🔒 Bot mode changed to *PRIVATE*\n\nOnly owner can use commands now.' });
            }
            
            if (mode === 'public' || mode === 'pub') {
                if (!config.selfMode) {
                    return sock.sendMessage(from, { text: '🌐 Bot is already in *PUBLIC* mode.\nEveryone can use commands.' });
                }
                
                // Update config
                updateConfig('selfMode', false);
                config.selfMode = false;
                return sock.sendMessage(from, { text: '🌐 Bot mode changed to *PUBLIC*\n\nEveryone can use commands now.' });
            }
            
            return sock.sendMessage(from, { text: '❌ Invalid mode!\nUsage: .mode <private/public>' });
            
        } catch (error) {
            console.error('Mode command error:', error);
            await sock.sendMessage(from, { text: '❌ Error changing bot mode.' });
        }
    }
};
