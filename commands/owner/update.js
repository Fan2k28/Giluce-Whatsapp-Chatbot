    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const req = client.get(url, {
                headers: {
                    'User-Agent': 'GiluceBot-Updater/1.0',
                    'Accept': '*/*'
                }
            }, res => {
...
module.exports = {
    ...
    async execute(sock, msg, args, context) {
        ...
        if (!zipUrl) {
            return sock.sendMessage(from, { text: '❌ No update URL configured. Set updateZipUrl in config.js or pass a URL: `.update <zip_url>`' });
        }

        try {
            await sock.sendMessage(from, { text: '🔄 Updating the bot, please wait…' });
